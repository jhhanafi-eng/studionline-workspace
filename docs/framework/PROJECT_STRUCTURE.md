# STUDIONLINE GAS Framework Blueprint: Project Structure

## Purpose
Canonical workspace and Apps Script project layout. This document defines architecture only; it does not provide implementation code, HTML, Apps Script source, or business application behavior.
...
- `ProjectStructureService.initialize(context)` for module startup when applicable.
- `ProjectStructureService.validate(input)` for contract validation when applicable.
- `ProjectStructureService.execute(request)` for the module's primary operation when applicable.
- `ProjectStructureService.describe()` for metadata, version, dependency, and capability reporting.
...
```text
apps/                 # future product or client applications only
...
shared/               # shared contracts and neutral assets
templates/            # starter layouts, not runtime dependencies
```
