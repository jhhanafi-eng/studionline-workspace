# STUDIONLINE GAS Framework Blueprint: Router Module

## Purpose
Route resolution for Apps Script SPA screens and server actions. This document defines architecture only; it does not provide implementation code, HTML, Apps Script source, or business application behavior.
...
## Folder Location
Recommended folder: `core/routing/`
...
- `RouterModuleService.initialize(context)` for module startup when applicable.
- `RouterModuleService.validate(input)` for contract validation when applicable.
- `RouterModuleService.execute(request)` for the module's primary operation when applicable.
- `RouterModuleService.describe()` for metadata, version, dependency, and capability reporting.
...
## Dependencies
- Auth, Configuration, Logger, Validation.
