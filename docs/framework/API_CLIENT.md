# STUDIONLINE GAS Framework Blueprint: API Client Module

## Purpose
External REST and AI integration boundary. This document defines architecture only; it does not provide implementation code, HTML, Apps Script source, or business application behavior.
...
## Folder Location
Recommended folder: `core/api/`

## Files
- `ApiClient.gs`
- `HttpRequestBuilder.gs`
- `RetryPolicy.gs`
- `ApiAdapters.gs`

## Public Functions
Public functions are architectural contracts, not generated code:
- `APIClientModuleService.initialize(context)` for module startup when applicable.
- `APIClientModuleService.validate(input)` for contract validation when applicable.
- `APIClientModuleService.execute(request)` for the module's primary operation when applicable.
- `APIClientModuleService.describe()` for metadata, version, dependency, and capability reporting.
...
## Dependencies
- Configuration, Logger, Error Handling, Cache.
