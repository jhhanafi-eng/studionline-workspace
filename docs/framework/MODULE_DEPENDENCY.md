# STUDIONLINE GAS Framework Blueprint: Module Dependency

## Purpose
Dependency direction, module contracts, and isolation rules. This document defines architecture only; it does not provide implementation code, HTML, Apps Script source, or business application behavior.
...
- `ModuleDependencyService.initialize(context)` for module startup when applicable.
- `ModuleDependencyService.validate(input)` for contract validation when applicable.
- `ModuleDependencyService.execute(request)` for the module's primary operation when applicable.
- `ModuleDependencyService.describe()` for metadata, version, dependency, and capability reporting.
...
| --- | --- | --- |
| UI | Router, public server contracts | Database internals, secrets, raw Google services |
| Router | Auth, Configuration, Logger, service contracts | Business internals, spreadsheet ranges |
| Services | Validation, Database contracts, API Client, Logger | UI widgets, raw HTML templates |
| Database | Configuration, Logger, Cache, Validation | UI, product workflows |
| API Client | Configuration, Logger, Cache, Error Handling | UI, database schema internals |
| Utilities | Constants | Any side-effecting service |
```mermaid
graph TD
  App --> UI
  App --> Services
